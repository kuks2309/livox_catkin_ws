...
class SerialReaderThread(threading.Thread):
	def __init__(self, motor_control, exit_event=None):
		...
		self.debug_flag = False  # debug_flag 초기화
			...
			try:
				if self.motor_control.serial_port and self.motor_control.serial_port.is_open:
					if self.motor_control.serial_port.in_waiting > 0:
						...
						elif self.motor_control.serial_command_type == 13:  # alarm 읽기
							data_length = 7  # ID(1) + FC(1) + BC(1) + Data(2) + CRC(2)
						...
						if self.motor_control.serial_command_type == 0:  # 모니터링 명령
							...
							self.handle_read_alarm_response(data_length)
	...
	def handle_read_alarm_response(self, data_length):
		"""레지스터 읽기 응답 처리"""
		if self.debug_flag:
			print("Register read response received: ", end='')
			for i in range(data_length):
		...
		if (self.read_buf[1] == 0x03 and  # Function code
			...
			if (self.read_buf[5] == (crc & 0xFF) and 
				...
				if value == 0x0000:
					...
					print(f"Alarm value: {value} (0x{value:04X})")
...
class MotorControl:
	...
	def read_alarm(self, motor_id):
		""" warning 레지스터 값 읽기"""	
		if self.serial_port and self.serial_port.is_open:
			protocol = bytearray([0] * 8)
			self.serial_command_type = 13  # 읽기 명령용 command type
			...
			if self.serial_port and self.serial_port.is_open:
				...
				protocol[2] = 0x00      # Register address high byte
				protocol[3] = 0x0E		# Register address low byte
				protocol[4] = 0x00      # Number of registers high byte
				...
				# 데이터 출력
				print(f"Reading alarm register 0x0E")
				print("Send: ", end='')
				for byte in protocol:
					print(f"0x{byte:02X} ", end="")
				print()
	...
	def move_incremental_position(self, motor_id, increment_pos):
		"""증분(상대) 위치로 이동하는 명령어 전송"""
		if self.serial_port and self.serial_port.is_open:
			protocol = bytearray([0] * 8)
			self.serial_command_type = 8  # Incremental position move command type
			
			protocol[0] = motor_id        # Device ID
			protocol[1] = 0x78            # Function code for incremental move
			
			# 음수인 경우 2의 보수 계산
			if increment_pos < 0:
				increment_pos = (1 << 32) + increment_pos  # 32비트 2의 보수
				
			protocol[2] = (increment_pos >> 24) & 0xFF  # Position highest byte
			protocol[3] = (increment_pos >> 16) & 0xFF  # Position high byte  
			protocol[4] = (increment_pos >> 8) & 0xFF   # Position low byte
			protocol[5] = increment_pos & 0xFF          # Position lowest byte
			...
			# 디버그 출력
			print("Send incremental move command: ", end='')
			for byte in protocol:
				print(f"0x{byte:02X} ", end="")
			print()
...
class WindowClass(QDialog, form_class):
	...
	def __init__(self):
		...
		self.lineEdit_Timer_Interval.setText("500")
		
		# Motor Rotation Angle 관련 변수
		self.textEdit_Scale_Encoder_Angle1.setText("16400")   #("16444.444")
		self.textEdit_Scale_Encoder_Angle2.setText("16400")
		self.textEdit_Scale_Encoder_Angle3.setText("16400")
		...
		# Timer 관련 변수
		self.checkBox_Timer_Enable.setChecked(False)
		self.checkBox_Timer_Enable.stateChanged.connect(self.timer_enable_function)
		...
		self.pushButton_Read_Alarm.clicked.connect(self.read_alarm_function)
		...
		self.pushButton_Set_Motor_Angle1.clicked.connect(self.set_target_angle)
		self.pushButton_Set_Motor_Angle2.clicked.connect(self.set_target_angle)
		self.pushButton_Set_Motor_Angle3.clicked.connect(self.set_target_angle)
		
		self.pushButton_Move_Motor_Encoder1.clicked.connect(self.set_target_encoder)
		self.pushButton_Move_Motor_Encoder2.clicked.connect(self.set_target_encoder)
		
		self.pushButton_Set_Motor_Rel_Angle1.clicked.connect(self.set_target_relative_encoder)
		...
		# debug 관련
		self.checkBox_UART_Debug.setChecked(False)
		self.checkBox_UART_Debug.stateChanged.connect(self.set_uart_debug)
	...
	def timer_enable_function(self):
		"""타이머 활성화"""
		if self.checkBox_Timer_Enable.isChecked():
			self.timer = QTimer()
			self.timer.timeout.connect(self.timer_function)
			self.timer.start(int(self.lineEdit_Timer_Interval.text()))
		else:
			self.timer.stop()

	def timer_function(self):
		self.read_alarm_function()
		print("Timer function called")

   
	def set_uart_debug(self, state):
		"""UART 디버그 모드 설정"""
		self.debug_flag = bool(state)
		if self.serial_reader:
			self.serial_reader.debug_flag = self.debug_flag
	...
	def read_alarm_function(self):
		"""Read alarm register (0x0E) from motor"""
		try:
			if self.is_connected:
				# Get Motor ID from Motor enabled text box
				motor_id = int(self.textEdit_Motor_Enable_ID.toPlainText())

				# Send alarm read command
				if self.motor_control.read_alarm(motor_id):
					print(f"Reading alarm status for Motor ID {motor_id}...")
				else:
					QMessageBox.warning(self, "Warning", "Failed to read alarm status")
			else:
				QMessageBox.warning(self, "Warning", "Please connect to serial port first")
		except ValueError:
			QMessageBox.critical(self, "Error", "Please enter a valid Motor ID")
		except Exception as e:
			QMessageBox.critical(self, "Error", f"Error reading alarm status: {str(e)}")
	...
	def set_target_encoder(self):
		...
		try:
			if self.is_connected:
				...
				if current_tab == 0:  # motor control 3 탭인 경우
					motor_id = int(self.textEdit_Motor_ID1.toPlainText())
					target_encoder  = int(self.textEdit_Set_Motor_Target_Encoder1.toPlainText())
					
					
					
					print("target_pos1", target_encoder)
				
				elif current_tab == 1:  # motor control 3 탭인 경우
					motor_id = int(self.textEdit_Motor_ID2.toPlainText())
					target_encoder  = int(self.textEdit_Set_Motor_Target_Encoder2.toPlainText())
					
				
					print("target_pos2", target_encoder)
				
				elif current_tab == 2:  # motor control 3 탭인 경우
					motor_id = int(self.textEdit_Motor_ID3.toPlainText())
				
					target_pos = int(target_angle*scale_factor)
					print("target_pos3", target_pos)
				else:
					QMessageBox.warning(self, "Warning", "Please select motor control 3 tab")
					return
					
				
				if self.motor_control.move_to_target_position(motor_id, target_encoder):
					print(f"Moving motor {motor_id} to position {target_encoder}")
				else:
			...
			QMessageBox.critical(self, "Error", f"Error moving to encoder: {str(e)}")
	...
	def set_target_angle(self):
		...
		try:
			if self.is_connected:
				...
				if current_tab == 0:  # motor control 3 탭인 경우
					motor_id = int(self.textEdit_Motor_ID1.toPlainText())
					scale_factor = float(self.textEdit_Scale_Encoder_Angle1.toPlainText())
					target_angle = float(self.textEdit_Set_Motor_Angle1.toPlainText())
					
					print("target angle1",target_angle)
					
					target_pos = int(13150 - target_angle*scale_factor)
					print("target_pos1", target_pos)
				
				elif current_tab == 1:  # motor control 3 탭인 경우
					motor_id = int(self.textEdit_Motor_ID2.toPlainText())
					scale_factor = float(self.textEdit_Scale_Encoder_Angle2.toPlainText())
					target_angle = float(self.textEdit_Set_Motor_Angle2.toPlainText())
					
					print("target angle2",target_angle)
					
					target_pos = int(15350 + target_angle*scale_factor)
					print("target_pos1", target_pos)
				
				elif current_tab == 2:  # motor control 3 탭인 경우
					motor_id = int(self.textEdit_Motor_ID3.toPlainText())
					scale_factor = float(self.textEdit_Scale_Encoder_Angle3.toPlainText())
					target_angle = float(self.textEdit_Set_Motor_Angle3.toPlainText())
					
					print("target angle3",target_angle)
	...
	def set_target_relative_encoder(self):
		"""상대 엔코더 위치로 이동"""
		try:
			if self.is_connected:
				# 현재 선택된 탭 인덱스 가져오기
				...
				# Motor ID와 상대 이동량 가져오기
				if current_tab == 0:  # motor control 1 탭인 경우
					motor_id = int(self.textEdit_Motor_ID1.toPlainText())
					relative_encoder = int(self.textEdit_Set_Motor_Rel_Angle1.toPlainText())
					
					# 상대 위치 이동 명령 전송
					if self.motor_control.move_incremental_position(motor_id, relative_encoder):
						print(f"Moving motor {motor_id} by relative position {relative_encoder}")
						QMessageBox.information(self, "Success", f"Moving by relative position {relative_encoder}")
					else:
						QMessageBox.warning(self, "Warning", "Failed to send incremental move command")
				else:
					QMessageBox.warning(self, "Warning", "Please select motor control 1 tab")
			else:
				QMessageBox.warning(self, "Warning", "Please connect to serial port first")
		except ValueError:
			QMessageBox.critical(self, "Error", "Please enter valid numbers for Motor ID and relative position")
		except Exception as e:
			QMessageBox.critical(self, "Error", f"Error moving to relative position: {str(e)}")
