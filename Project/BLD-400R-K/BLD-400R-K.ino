...
int remote_robot_wheel_speed = 0;
int auto_robot_wheel_speed = 0;
int semi_robot_wheel_speed = 0;
int auto_robot_steering_angle = 0;
...
int16_t convertChannelToWheelSpeed(int ch_data)
{
  ...
  else if (ch_data < 950)
  {
    ...
    */
    ...
    robot_wheel_speed = map(ch_data, 950,200,0,1500);
    /*
    digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
    ...
    */
    if(robot_wheel_speed>=1500)  robot_wheel_speed = 1500;
...
int16_t convertChannelToSteeringAngle(int ch_data)
{
  ...
  else if (ch_data < 950)
  {
    ...
    */
    ...
    steering_angle = map(ch_data, 950, 250, 0, 45);

    if(steering_angle >=62 ) steering_angle = 90;
    /*
    digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
    Serial3.print("steering_angle : "); Serial3.println(steering_angle);
    Serial3.flush();  // 전송 완료 대기
    digitalWrite(TX_485_3, LOW);    // 송신 모드로 전환
    */
  }
  ...
  else if (ch_data > 1050)
  {
    ...
    */
    steering_angle = map(ch_data, 1050, 1700, 0, -45);
...
void start_front_steering_control(float target_angle)
{
  ...
  if(robot_run_mode == 0) 
  {
    m_speed = remote_robot_wheel_speed;
    /*
    digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
    //Serial3.println("disable front wheel motor");
    Serial3.print("target angle : "); Serial3.println(target_angle);
    Serial3.print("robot speed : "); Serial3.println(m_speed);
    Serial3.flush();  // 전송 완료 대기
    digitalWrite(TX_485_3, LOW);    // 송신 모드로 전환      
    */  
  }
  else if(robot_run_mode == 1 )
  {
    m_speed = semi_robot_wheel_speed;       
  }

  else if(robot_run_mode == 2) 
  {

    m_speed = auto_robot_wheel_speed;       
  }

  // 휠 모터 출력 비활성화 (아직 비활성화되지 않았다면)
  if(!front_steering_state.wheels_disabled)
  {
    if(fabs(m_speed) < 150)
    {      
      /*
...
void start_rear_steering_control(float target_angle)
{
  ...
  if(robot_run_mode == 0) 
  {
    ...
    /*
    digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
    //Serial3.println("disable rear wheel motor");
    Serial3.print("robot speed : "); Serial3.println(m_speed);
    Serial3.print("target angle : "); Serial3.println(target_angle);
    Serial3.flush();  // 전송 완료 대기
    digitalWrite(TX_485_3, LOW);    // 송신 모드로 전환
  ...
  // 휠 모터 출력 비활성화 (아직 비활성화되지 않았다면)
  if(!rear_steering_state.wheels_disabled)
  {
...
bool check_steering_status()
{
  ...
  if(run_mode == 0)  // 임시 방편으로... car mode 일때는 전방 횔 해제
  {
    
    if(fabs(auto_robot_wheel_speed) > 200)
    {
...
void disable_front_wheel_motors()
{
  ...
  if(DEBUG_ENABLE) 
  {
    Serial.println("Front wheel motors disabled");
  }
}
...
void disable_rear_wheel_motors()
{
  ...
  if(DEBUG_ENABLE) 
  {
    Serial.println("Rear wheel motors disabled");
  }
}
...
void enable_all_wheel_motors()
{
  /*
  digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
  Serial3.println("enable all motors");
  Serial3.flush();  // 전송 완료 대기
  digitalWrite(TX_485_3, LOW);    // 송신 모드로 전환
  */
  for(int motor_id = 1; motor_id <= 4; motor_id++)
...
void loop()
{
  ...
  if (currentTime - preTime > interval)
  {
    ...
    {        
        ...
        remote_robot_wheel_speed = base_speed = robot_wheel_speed = convertChannelToWheelSpeed(ch1_data);
        ...
        */
        ...
        if(steering_mode == CRAB_MODE)
        {
          ...
          crabSteeringModel(remote_robot_wheel_speed, robot_steering_angle, calc_results); 
        ...
        {         
          ...
          {            
            pivotTurnSteeringModel(remote_robot_wheel_speed,calc_results);
            // printResults(calc_results);
            /*  
            digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
          ...
          else
          {
            ...
            symmetricDualPivotSteeringModel(remote_robot_wheel_speed, robot_steering_angle, -robot_steering_angle, calc_results); 
            //printResults(calc_results);
    ...
    {
      ch1_data = (receiveProtocol.ch1_H << 8) | receiveProtocol.ch1_L;
      semi_robot_wheel_speed = robot_wheel_speed = convertChannelToWheelSpeed(ch1_data);
    ...
    {   
      /*
      digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
      Serial3.print("steering angle : "); 
      Serial3.print(steering_angle[0]); Serial3.print(" ");
      Serial3.print(steering_angle[1]); Serial3.print(" ");
      Serial3.print(steering_angle[2]); Serial3.print(" ");
      Serial3.print(steering_angle[3]); Serial3.print(" ");
      ...
      */
      ...
      {        
        float calc_results[6] = {0.0,};
        // car mode 에서 steering angle을 구함
        carSteeringModel2(auto_robot_wheel_speed, steering_angle[0], calc_results);              
        //printResults(calc_results);
        int16_t wheel_motor_speed_new[4] = { 0, 0, 0, 0};
        ...
        /*
        digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
        Serial3.print("wheel_motor_speed_new : "); 
        Serial3.print(wheel_motor_speed_new[0]); Serial3.print(" ");
        Serial3.print(wheel_motor_speed_new[1]); Serial3.print(" ");
        Serial3.print(wheel_motor_speed_new[2]); Serial3.print(" ");
        Serial3.print(wheel_motor_speed_new[3]); Serial3.print(" ");
      ...
      { 
        ...
        */
        ...
        float calc_results[6] = {0.0,};
        crabSteeringModel(auto_robot_wheel_speed, robot_steering_angle, calc_results); 
        //printResults(calc_results);
        wheel_motor_speed_new[0] =   calc_results[2];    
        wheel_motor_speed_new[1] =   calc_results[3];
        wheel_motor_speed_new[2] =   calc_results[4];
      ...
      {        
        if( (steering_angle[0] == 90) ||  (steering_angle[0] == -90) )
        {
          ...
          int16_t wheel_motor_speed_new[4] = { 0, 0, 0, 0};
          float calc_results[6] = {0.0,};          
          pivotTurnSteeringModel(auto_robot_wheel_speed,calc_results);
        ...
        {         
          ...
          float calc_results[6] = {0.0,};
          symmetricDualPivotSteeringModel(auto_robot_wheel_speed,steering_angle[0],steering_angle[1],calc_results );
          //printResults(calc_results);
          wheel_motor_speed_new[0] =   calc_results[2];    
          wheel_motor_speed_new[1] =   calc_results[3];
          wheel_motor_speed_new[2] =   calc_results[4];
    ...
    if(run_mode == 0 )
    {
      /*
      digitalWrite(TX_485_3, HIGH);    // 송신 모드로 전환
      Serial3.print("robot speed : "); Serial3.print(base_speed);
      ...
      Serial3.print(" run mode : "); Serial3.print("Remote control");
      Serial3.println(" "); 
      Serial3.flush();  // 전송 완료 대기
      digitalWrite(TX_485_3, LOW);    // 송신 모드로 전환
      */
    ...
    if( (data[i+0] == '#') && (data[i+1] == 'R') && (data[i+2] == 'T') && (data[i+10] == '*'))
    {
      ...
      if(receivedCRC == calculatedCRC)
      {
        ...
        if(data[i+3] == 0x00)
        {
          ...
          auto_robot_steering_angle = steering_angle1;
          int16_t steering_angle_new[2] = { 0, 0};
          if(data[i+7] == 0x01)
          {
